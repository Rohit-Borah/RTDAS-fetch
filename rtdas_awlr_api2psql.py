...
TABLE_NAME = os.getenv("AWLR_TABLE")
...
def clean_record(record):
    ...
    filtered = {
        ...
        "waterLevel": record.get("waterLevel"),
    ...
    for rec in records:
        try:
            cur.execute(f"""
                ...
                (uuid, inputDate, batteryLevel, waterLevel, stationID)
                VALUES (%s,%s,%s,%s,%s)
                ON CONFLICT (stationID, inputDate) DO NOTHING;
            """, (
                rec["uuid"], rec["inputDate"], rec["batteryLevel"], rec["waterLevel"], rec["stationID"]
            ))
